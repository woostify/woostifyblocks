			...
			{list.map((item) => {
				...
				return (
					<Fragment key={item.id}>
						...
						<ListItems {...props} list={item.children} depth={depth + 1} />
					</Fragment>
				);
			})}
		</>
		...
		>
			...
			{selectedCount > 0 ? (
				<ul>
					{selected.map((item, i) => (
						<li key={i}>
							<Tag label={item.name} id={item.id} remove={onRemove} />
						</li>
					))}
				</ul>
			) : null}
...
const ListItemsContainer = ({
	...
	if (filteredList.length === 0) {
		return (
			<div className="woocommerce-search-list__list is-not-found flex items-center">
	...
	return (
		<ul className="woocommerce-search-list__list hiddenScrollbar rounded-lg">
			<ListItems
				list={filteredList}
				selected={selected}
...
export const SearchListControl = (
	...
	return (
		...
		>
			<SelectedListItems {...props} onRemove={onRemove} messages={messages} />
