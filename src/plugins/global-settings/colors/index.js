...
const WCBGlobalColors = props => {
	...
	const handleUpdateColors = newColors => {
		...
		saveTimeout = setTimeout( () => {
			const settings = new models.Settings( { wcb_global_colors: [ updatedColors ] } )
			settings.save()
		}, 300 )
					...
					{ selectedIndex === index &&
						...
						>
							...
							<BaseControl
								...
								label={ __( 'Color Name', 'woostify-block' ) }
							>
								<input
									className="components-text-control__input"
									id="wcb-color-picker-text-name"
									onChange={ event => handleOnChangeColorName( event.target.value ) }
									value={ color.name || '' }
								/>
							</BaseControl>
							<Button
