	...
	loadPromise.then( () => {
		...
		settings.fetch().then( response => {
			...
			if ( ( _colors || [] ).some( color => color.fallback && color.colorVar ) ) {
				...
				wcbColors = colors.filter( ( { slug } ) => slug.match( /^wcb-global-color/ ) )
				wcbColorSlugs = wcbColors.map( color => color.slug )
			} else {
				colors = _colors || []
			}
			const defaultColors = colors.filter( ( { slug } ) => ! wcbColorSlugs.includes( slug ) )
