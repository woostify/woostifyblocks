...
const Edit = props => {
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    ...
    setCurrentNumber(parseInt(general_layout?.startNumber) || 0);
  }, [general_layout?.startNumber, general_layout?.endNumber, general_layout?.animationDuration, general_layout?.type]);

  // Setup Intersection Observer for scroll trigger in editor
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
  ...
  (0,react__WEBPACK_IMPORTED_MODULE_0__.useEffect)(() => {
    ...
    const startNumber = parseInt(general_layout?.startNumber) || 0;
    const duration = parseInt(general_layout?.animationDuration) || 1500;
    if (startNumber === targetNumber) {
      setCurrentNumber(targetNumber);
      return;
    }
    const incrementTime = duration / Math.abs(targetNumber - startNumber);
    let current = startNumber;
    setCurrentNumber(current);
    const interval = setInterval(() => {
      current += startNumber < targetNumber ? 1 : -1;
      setCurrentNumber(current);
      if (startNumber < targetNumber && current >= targetNumber || startNumber > targetNumber && current <= targetNumber) {
        setCurrentNumber(targetNumber);
        clearInterval(interval);
      }
    }, incrementTime);
    return () => clearInterval(interval);
  }, [shouldAnimate, general_layout?.animationDuration, general_layout?.startNumber, general_layout?.endNumber]);
